    ...
    ui(new Ui::Fee)
{
    ...
    m_model.setHorizontalHeaderLabels(QStringList{"Fno","Fnumber","Fdate","DGno","Rno","Pno","Fsum"});
    ui->tableView->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);
    titleList=QStringList{"Fno","Fnumber","Fdate","DGno","Rno","Pno","Fsum"};
    this->icols=7;
}
...
void Fee::on_input_clicked()
{
    m_model.clear();
    m_model.setHorizontalHeaderLabels(QStringList{"Fno","Fnumber","Fdate","DGno","Rno","Pno","Fsum"});
    qDebug()<<"fee导入";
    QSqlQuery q(SqlMgr::getInstance()->m_db);
    QString str=QString("select * from `cs2344.fee` ");
...
void Fee::on_delete_2_clicked()
{
    ...
    QSqlQuery q(SqlMgr::getInstance()->m_db);
    QString str1=QString("delete  from `cs2344.fee` where Fno='%1' ").arg(ziduanList[0]);
...
void Fee::on_add_clicked()
{
    ...
    QSqlQuery q(SqlMgr::getInstance()->m_db);
    QString str=QString("insert into `cs2344.fee` values('%1','%2','%3','%4','%5','%6','%7') ").arg(ziduanList[0]).arg(ziduanList[1])
            .arg(ziduanList[2]).arg(ziduanList[3]).arg(ziduanList[4]).arg(ziduanList[5]).arg(ziduanList[6]);
    bool ret=q.exec(str);
    if(ret){
...
void Fee::on_revise_clicked()
{
    ...
    QSqlQuery q(SqlMgr::getInstance()->m_db);
    QString str=QString(" update `cs2344.fee` set Fnumber='%1',Fdate='%2',DGno='%3',Rno ='%4',Pno='%5',Fsum='%6' where Fno='%7' ")
            .arg(ziduanList[1])
            .arg(ziduanList[2]).arg(ziduanList[3]).arg(ziduanList[4]).arg(ziduanList[5]).arg(ziduanList[6]).arg(ziduanList[0]);
    bool ret=q.exec(str);
    if(ret){
        on_input_clicked();
    }
